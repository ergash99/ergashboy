...
*/
@Service
public class DalolatnomaService {
    @PersistenceContext
    ...
    public List<Dalolatnoma> getAllList(Integer pageNumber) {
        int perPage = Constants.LIST_PER_PAGE;
        int skipRows = pageNumber * perPage;
        try {
            Query query;
            query = entityManager.createNamedQuery("Dalolatnoma.findALL", Dalolatnoma.class);
    ...
    @SuppressWarnings("unchecked")
    @Transactional(readOnly = true)
    public Long countDalolatnoma(){
        try {
            TypedQuery<Long> query = (TypedQuery<Long>) entityManager.createNamedQuery("Dalolatnoma.countAll", Long.class);
